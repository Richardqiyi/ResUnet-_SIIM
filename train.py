...
def main(hp, num_epochs, resume, name):
    ...
    # decay LR
    # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.333)
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=200, eta_min=1e-7)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, data in enumerate(loader):
            ...
            if step % hp.logging_step == 0:
                writer.log_training(train_loss.avg, train_acc.avg, current_lr, step, epoch)
